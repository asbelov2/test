...
function sostavChisla(massivChisel, chislo) {
	...
	return checkCombinations(massivChisel, chislo);
}

// Получаем все комбинации и фильтруем их в результат
function checkCombinations(massivChisel, chislo) {
    let combs = []
    let len = massivChisel.length
    for(let k = 1; k<len+1; k++) {
        combs = combs.concat(getCombinations(massivChisel, k))
    }
    return combs.filter(comb => comb.reduce((a,b) => a + b) === chislo)
}

// Функция для получения всех комбинаций без повторений взятая из интернета
const getCombinations = (arr, k, withRepetition = false) => {
    const combinations = []
